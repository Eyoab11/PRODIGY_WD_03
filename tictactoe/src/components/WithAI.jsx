...
const WithAI = () => {
    ...
    const renderCell = (row, col) => {
        // Check if the current cell is a winning cell
        const isWinningCell = winningCells.some(cell => cell[0] === row && cell[1] === col);
    
        // Check if the current cell is part of the primary diagonal
        const isPrimaryDiagonal = row === col;
    
        // Check if the current cell is part of the secondary diagonal
        const isSecondaryDiagonal = row + col === 2;
    
        // Define the base cell class
        let cellClassName = 'w-24 h-24 bg-white border-2 border-gray-800 text-2xl font-bold';
    
        // Add additional classes based on conditions
        if (isWinningCell) {
            cellClassName += ' bg-blue-200'; // Apply winning cell background color
        }
        if (isPrimaryDiagonal) {
            cellClassName += ' diagonalLine1'; // Apply primary diagonal line style
        }
        if (isSecondaryDiagonal) {
            cellClassName += ' diagonalLine2'; // Apply secondary diagonal line style
        }
    
        // Return the JSX for the cell
        return (
            <button
                key={`${row}-${col}`}
